...
각도 루프만으로 약 1분간 균형을 유지하며, 그 위에 **엔코더 기반 바퀴 속도 루프**를 얹을 수 있습니다.
...
| 모터 드라이버 | TB6612FNG | |
| 모터 | DFRobot FIT0450 × 2 | 6V, 160 RPM, 120:1 감속, 엔코더 내장 |
...
| 모터 출력 | 연결 |
|---|---|
| AO1 / AO2 | A채널 모터 빨강 / 검정 |
| BO1 / BO2 | B채널 모터 빨강 / 검정 |

모터의 빨강·검정 선은 꼬인 채로 두세요. 풀면 PWM 노이즈가 커집니다. 좌우 모터가 서로 마주 보게 달렸으므로 회전 방향은 펌웨어(`LSIGN`/`RSIGN`)에서 맞춥니다.

> 펌웨어의 A채널("L")과 B채널("R")이 실제 좌우와 반대로 물려 있어도 **균형에는 영향이 없습니다.** 두 모터에 항상 같은 값을 보내기 때문입니다. 다만 `TEST L` / `TEST R`이 반대쪽 바퀴를 돌리니 점검할 때 헷갈리지 않도록 주의하세요.

### 엔코더 (8선)

| 엔코더 | 선 | Nucleo | 타이머 |
|---|---|---|---|
| A채널 쪽 | 초록 (A) | **D12** (PA6) | TIM3_CH1 |
| | 파랑 (B) | **D11** (PA7) | TIM3_CH2 |
| B채널 쪽 | 초록 (A) | **D6** (PE9) | TIM1_CH1 |
| | 파랑 (B) | **D5** (PE11) | TIM1_CH2 |
| 공통 | 빨강 (VCC) | **3V3** | 6V 절대 금지 |
| | 검정 (GND) | Nucleo **GND 핀** | 모터 접지 경로와 별개 |

엔코더 케이블에도 빨강·검정이 있어서 모터 전원선과 헷갈리기 쉽습니다. **꼬인 2선이 모터 전원, 납작한 4선이 엔코더**입니다.

> 아두이노 헤더의 `A0`/`A1`은 `PA0`/`PA1`이 **아닙니다.** `A0`은 PA3, 즉 이 회로에서 TB6612의 STBY로 이미 쓰고 있는 핀입니다. 엔코더를 거기 꽂으면 STM32 출력과 맞부딪칩니다. 그래서 양쪽 엔코더를 모두 번호가 명확한 D 핀으로 배치했습니다.
...
각도만 보는 제어는 원리적으로 오래 버틸 수 없습니다. 모터에 주는 듀티는 사실상 **바퀴 속도**를 정하는데, 기울기를 바로잡을 때마다 한쪽 방향으로 속도가 조금씩 쌓입니다. 결국 바퀴가 최고 속도(160 RPM)에 닿으면 더 가속할 수 없어 다음 기울기를 못 쫓아가고 넘어집니다. 이것이 1~2분 한계의 정체입니다.

외부 루프는 바퀴 속도를 보고 **목표각을 그 반대로 기울입니다.**

```
앞으로 흐름 (SPD > 0)  →  목표각을 뒤로 (OFF < 0)
                       →  로봇이 살짝 뒤로 기울며 속도가 줄어듦
                       →  양방향 가속 여유 회복
```

**위치는 일부러 제어하지 않습니다.** 제자리 복귀보다 "넘어지지 않는 것"이 목표라, 로봇은 천천히 떠다니되 속도 여유는 유지합니다.

- 속도는 좌우 카운터 차분의 평균입니다. 회전 성분은 상쇄됩니다.
- 보정량은 **±3°로 제한**됩니다. 제한이 없으면 바깥 루프가 "보정"하다가 로봇을 눕힙니다.
- 속도 추정에는 **느린 저역통과 필터**(`KVF`, 기본 0.98 ≈ 200 ms)가 걸려 있습니다. 바깥 루프는 안쪽 루프보다 **둔해야** 합니다. 빠르면 로봇 자신의 까딱임을 쫓아다니며 목표각을 흔들어 각도 루프와 싸웁니다.

**기본값은 `KV = 0`, 즉 꺼진 상태입니다.** 배선 후 `SPD`가 올바른 부호로 읽히는지 확인한 뒤 `KV`를 조금씩 올리세요. 속도를 줄이려면 먼저 속도를 높여야 하는 구조라(뒤로 기울려면 바퀴를 앞으로 굴려야 함), `KV`가 과하면 쉽게 출렁입니다.
...
복구에 성공하면 `I2C: bus recovered (err=0x04, total=7)`처럼 출력하고 계속 균형을 잡습니다. 정상 구간 없이 복구가 5회를 넘으면 포기하고 `MPU_FAULT`로 멈춥니다.

**리셋 직후에도 같은 해방 절차를 먼저 거칩니다.** 리셋이 I2C 전송 도중에 걸리면 MPU가 SDA를 붙잡은 채 남는데, MCU만 재시작해서는 풀리지 않습니다. 이 절차가 없으면 리셋할 때마다 `MPU_FAULT`로 떨어져 전원을 뽑아야 합니다.

`err`는 HAL이 보고한 실패 원인이고, 어디를 봐야 할지 알려줍니다.

| 값 | 의미 | 가리키는 곳 |
|---|---|---|
| `0x04` AF | 슬레이브가 ACK를 안 보냄 | 모듈 또는 배선 |
| `0x01` BERR | START/STOP 위치가 깨짐 | 노이즈 |
| `0x02` ARLO | 중재 실패 | 노이즈 또는 충돌 |
| `0x20` TIMEOUT | 버스가 붙잡혀 있음 | 슬레이브가 물림 |

텔레메트리의 `REC`은 부팅 후 누적 복구 횟수입니다. 배선이나 부품을 바꿔가며 **숫자로 비교**할 때 씁니다.
...
| `KV n` | 속도 외부 루프 게인 (0이면 끔) | −5‥5 |
| `KVF n` | 속도 필터 계수 (클수록 느림) | 0‥0.999 |
| `ELSIGN n` `ERSIGN n` | 엔코더 계수 방향 | ±1 |
| `ENC` | 엔코더 카운터와 핀 레벨 출력 (진단용) | |

텔레메트리는 200 ms마다 한 줄씩 나옵니다. 부동소수를 쓰지 않으려고 모두 정수로 찍습니다.

```
S=ARMED ANG=-37 GYR=552 OUT=15 TGT=-50 SPD=120 OFF=-60 REC=0 KV=100 KVF=980
        KP=4500 KI=0 KD=250 DEAD=130 MAX=799 SS=-1 LS=1 RS=-1 ES=1/-1
```

| 필드 | 단위 | 예시 해석 |
|---|---|---|
| `ANG` `TGT` `OFF` | 0.01° | `-37` = −0.37° |
| `GYR` | 0.01 dps | `552` = 5.52 dps |
| `OUT` | PWM 카운트 | 상한 `MAX` |
| `SPD` | 0.01 counts/tick | `120` = 1.20 |
| `REC` | 회 | 부팅 후 누적 I2C 복구 |
| `KP` `KI` `KD` | ×100 | `4500` = 45.00 |
| `KV` `KVF` | ×1000 | `100` = 0.100 |
| `SS` `LS` `RS` `ES` | ±1 | 센서 / 좌 / 우 / 엔코더(좌·우) 부호 |
...
| `KV` | 0 | 외부 루프는 꺼진 채로 출하. 배선 확인 후 직접 올림 |
| `KVF` | 0.98 | 약 200 ms. 안쪽 루프보다 느려야 함 |
| `ELSIGN` / `ERSIGN` | +1 / −1 | 모터가 마주 보므로 두 엔코더는 반대로 셈 |
...
7. **엔코더 읽기** — `ENC`를 치고 바퀴를 손으로 굴리며 여러 번 더 칩니다. `L`과 `R`이 모두 변해야 합니다. 안 변하면 같이 출력되는 핀 레벨(`D12` `D11` `D6` `D5`)을 보세요. 전부 0이면 전원, 1에서 고정이면 신호선 문제입니다.
8. **속도 부호** — 로봇을 손으로 앞으로 밀 때 `SPD`가 양수여야 합니다. 아니면 `ELSIGN` / `ERSIGN`. 한쪽 바퀴씩 굴려 각각 같은 부호가 나오는지 보는 게 확실합니다.
9. **외부 루프** — `KV`를 작은 값부터(0.05 → 0.1 → 0.3) 올리며 `SPD`가 한 방향으로 계속 커지지 않는지 봅니다. `OFF`가 ±300(제한값)에 붙어 있으면 `KV`가 과한 것이고, 계속 0 근처면 약한 것입니다. 출렁이면 `KV`를 낮추기 전에 `KVF`를 0.99로 올려보세요.
...
| 한쪽 바퀴가 **한 방향으로만** 돔 | 그 채널의 방향 핀 한 가닥이 끊김 | `TEST L 250` / `TEST L -250`로 확인. 끊긴 입력이 뜬 상태가 되어 `L/L`(프리런)이나 `H/H`(브레이크) 조합이 만들어짐. `AIN1`·`AIN2`(D2·D4) 또는 `BIN1`·`BIN2`(D7·D8) 교체 |
...
**각도 루프만으로는 1~2분이 한계입니다.** 그 이유와 대책(엔코더 속도 루프)은 위의 "바퀴 속도 외부 루프"에 정리했습니다. 엔코더 하드웨어와 펌웨어는 들어가 있고, `KV`를 올려 켜는 것부터가 남은 튜닝입니다.

그 밖에 남은 것:

- **모터 단자에 0.1 µF 납땜** — 지금은 브레드보드 쪽에 꽂혀 있어 효과가 절반 이하입니다. 노이즈는 발생 지점에서 잡아야 합니다
- **TB6612 VM–GND에 470~1000 µF 전해** — 모터 기동 시 전압 강하 완화. 10 µF로는 두 자릿수 모자랍니다
- **알칼리 대신 NiMH** — 전지가 닳을수록 게인이 안 맞게 되는 문제 완화
- **HC-06 블루투스로 무선 튜닝** — USB 케이블이 로봇을 잡아당기지 않도록. 균형이 잡히면 로봇이 돌아다녀서 유선 튜닝이 어렵습니다
- **큰 바퀴** — 160 RPM이 고정이라 지름이 곧 최고 속도입니다. 대신 토크 여유는 줄어듭니다

## 배운 것

며칠에 걸친 디버깅에서 **코드가 원인이었던 적은 거의 없었습니다.** 실제 원인은 이랬습니다.

- MPU6050을 6V에 연결 (3.3V 부품)
- 배터리 −가 연결 안 됨
- `PWMA`가 헤더에서 한 칸 밀려 꽂힘
- 모터 리드선이 몸체 납땜부에서 떨어짐
- `STBY` 선이 빠져 있음
- 엔코더를 `PA0`/`PA1` 대신 `A0`/`A1`(= STBY 핀)에 연결
- 모터 접지와 센서 접지가 브레드보드 레일을 공유
- 방향 핀 한 가닥이 끊겨 한쪽 바퀴가 **한 방향으로만** 회전
- 케이블타이로 고정하다 MPU 모듈의 탄탈 커패시터를 압박

증상은 매번 "PID가 안 맞는 것"처럼 보였습니다. 게인을 만지기 전에 **부호 → 배선 → 전원 → 접지** 순으로 확인하는 편이 훨씬 빠릅니다.
